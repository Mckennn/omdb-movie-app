...
<div className="relative z-10">
  <Header onSearch={(q) => q && navigate(`/search?q=${encodeURIComponent(q)}`)} />
  ...
  {!loading && !error && (
    <>
      <Hero
        hero={hero}
        detail={heroDetail}
        onSimilarClick={() => hero && navigate(`/search?q=${encodeURIComponent(hero.Title)}`)}
      />
      <Carousel
        items={carousel}
        loading={false}
        onOpen={(title) => navigate(`/search?q=${encodeURIComponent(title)}`)}
        onReload={refresh}
      />
    </>
  )}
